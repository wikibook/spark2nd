...
import static org.apache.spark.sql.functions.grouping_id;
import static org.apache.spark.sql.functions.last_day;
...
public class DataFrameSample {
	...
	public static void main(String[] args) {
		...
		SparkSession spark = SparkSession.builder().appName("DataFrameSample").master("local[*]")
	...
	public static void createDataFrame(SparkSession spark, JavaSparkContext sc) {
		...
		String path = sparkHomeDir + "/data/mllib/images";
		boolean recursive = true;
		int numPartitions = 2;
		boolean dropImageFailures = true;
		double sampleRatio = 1.0;
		int seed = 0;

		Dataset<Row> imgdf = ImageSchema.readImages(path, spark, recursive, numPartitions, dropImageFailures,
				sampleRatio, seed);

		imgdf = imgdf.select(imgdf.col("image.origin"), imgdf.col("image.height"), imgdf.col("image.width"),
				imgdf.col("image.nChannels"), imgdf.col("image.mode"));
	...
	public static void runAliasEx(SparkSession spark, JavaSparkContext sc, Dataset<Row> df) {
		...
		Metadata meta = new MetadataBuilder().putString("desc", "this is the first column").build();
		Dataset<Row> dfm = spark.range(1).select(col("id").as("idm", meta));
		System.out.println(dfm.schema().fields()[0].metadata());
	}
	...
	public static void runWhenEx(SparkSession spark, JavaSparkContext sc) {
		...
		Column col = when(ds.col("id").mod(2).equalTo(0), "even").otherwise("add");
	...
	public static void runAggregateFunctions(SparkSession spark, Dataset<Row> df1, Dataset<Row> df2) {
		...
		scala.collection.mutable.Buffer<String> colNames = JavaConversions.asScalaBuffer(Arrays.asList("product"));

		df2.cube(df2.col("store"), df2.col("product")).agg(sum("amount"), grouping_id("store", colNames)).show();

		// grouping_id를 이용한 정렬
		Map<String, Object> naMap = new HashMap<>();
		naMap.put("store", "TOTAL");
		naMap.put("product", "-");

		df2.cube(df2.col("store"), df2.col("product"))
				.agg(sum("amount").as("sum"), grouping_id("store", colNames).as("gid")).filter("gid != '2'")
				.sort(asc_nulls_last("store"), col("gid")).na().fill(naMap).select("store", "product", "sum").show();
	}
	...
	public static void runCollectionFunctions(SparkSession spark) {
		...
		// split는 문자열을 지정한 delimiter로 분리하여 배열 타입의 컬럼을 생성하는 functions 메서드임(5.5.2.4.19절
		// 참조)
	...
	public static void runOtherFunctions(SparkSession spark, Dataset<Row> personDf) {
		...
		dd.select(dd.col("product"), dd.col("amount"), row_number().over(w1).as("rownum"), rank().over(w2).as("rank"))
				.show();
	}
	...
	public static void runUDF(SparkSession spark, Dataset<Row> df) {
		...
		// Seq<Column> cols =
		// JavaConversions.asScalaBuffer(Arrays.asList(df.col("job"))).toSeq();
		// df.select(df.col("name"), df.col("age"), df.col("job"),
		// fn1.apply(cols)).show(false);
	...
	public static void runJoin(SparkSession spark, Dataset<Row> ldf, Dataset<Row> rdf) {
		for (String joinType : "inner,outer,leftouter,rightouter,leftsemi".split(",")) {
			...
			// //JavaConversion를 사용하여 처리할 경우
			// Seq<String> joinCols =
			// JavaConversions.asScalaBuffer(Arrays.asList("word")).toSeq();
	...
	public static void runSave(SparkSession spark) {
		...
		df.write().format("parquet").bucketBy(20, "age").mode(SaveMode.Overwrite).saveAsTable("bucketTable");
		spark.sql("select * from bucketTable").show();
	}

	// 5.5.2.4.41.4절
	public static void runRepartitionByRange(SparkSession spark) {

		Dataset<Long> df = spark.range(0, 500, 7);

		Dataset<Long> df1 = df.repartition(5, df.col("id"));
		Dataset<Long> df2 = df.repartitionByRange(5, df.col("id"));

		System.out.println("Repartition ---------------");

		df1.foreachPartition(new ForeachPartitionFunction<Long>() {
			@Override
		...
		df2.foreachPartition(new ForeachPartitionFunction<Long>() {
			@Override
			public void call(Iterator<Long> it) throws Exception {
				StringBuffer buff = new StringBuffer();
				while (it.hasNext()) {
	...
	// 5.5.2.4.41.5절
	public static void runColRegex(SparkSession spark) {
		Product d1 = new Product("store2", "note", 20, 2000);
		Product d2 = new Product("store2", "bag", 10, 5000);
		Dataset<Row> df1 = spark.createDataFrame(Arrays.asList(d1, d2), Product.class);
		Dataset<Row> df2 = df1.toDF("store_nm", "prod_nm", "amount", "price");
		df2.select(df2.colRegex("`.*nm`")).show();
	}

	// 5.5.2.4.41.5절
	public static void runUnionByName(SparkSession spark) {
		Product d1 = new Product("store2", "note", 20, 2000);
		Product d2 = new Product("store2", "bag", 10, 5000);
		Dataset<Row> df = spark.createDataFrame(Arrays.asList(d1, d2), Product.class);
		Dataset<Row> df1 = df.toDF("store_nm", "prod_nm", "amount", "price");
		Dataset<Row> df2 = df1.select("price", "amount", "prod_nm", "store_nm");
		df1.union(df2).show();
		df1.unionByName(df2).show();
	}

	// 5.5.2.4.41.7절
	public static void runToJson(SparkSession spark) {
		Product d1 = new Product("store2", "note", 20, 2000);
		Product d2 = new Product("store2", "bag", 10, 5000);
		Dataset<Row> df = spark.createDataFrame(Arrays.asList(d1, d2), Product.class);
		df.select(to_json(struct(df.col("store").as("store_nm"), df.col("product").as("prod_nm"), df.col("amount"),
				df.col("price"))).as("value")).show(false);
	}

	// 5.5.2.4.41.7절
	public static void runFromJson(SparkSession spark) {

		String v1 = "{\"store_nm\":\"store2\", \"prod_nm\":\"note\", \"amount\":\"20\", \"price\":\"2000\"}";
		String v2 = "{\"store_nm\":\"store2\", \"prod_nm\":\"bag\", \"amount\":\"10\", \"price\":\"5000\"}";

		StructType schema = new StructType()
				.add("store_nm", DataTypes.StringType, true)
				.add("prod_nm", DataTypes.StringType, true)
				.add("amount", DataTypes.StringType, true)
				.add("price", DataTypes.StringType, true);

		Dataset<String> ds = spark.createDataset(Arrays.asList(v1, v2), Encoders.STRING());
		Dataset<Row> df = ds.toDF("value");
		df.select(from_json(df.col("value"), schema).as("value"))
		  .selectExpr("value.store_nm", "value.prod_nm", "value.amount", "value.price") 
		  .show(false);
	}
