...
object DataFrameSample {
  ...
  def createDataFrame(spark: SparkSession, sc: SparkContext) {
    ...
    // 3. 기존 RDD로부터 데이터프레임 생성 (ex5-18)
    val rdd = sc.parallelize(data)
    val df6 = spark.createDataFrame(rdd)
    val df7 = rdd.toDF

    // Product를 상속 클래스의 컬렉션을 이용하여 생성(5.5.1.2.1 노트)
    val _row1 = new Person2("hayoon", 7, "student")
    val _row2 = new Person2("sunwoo", 13, "student")
    val _row3 = new Person2("hajoo", 5, "kindergartener")
    val _row4 = new Person2("jinwoo", 13, "student")
    val _data = List(_row1, _row2, _row3, _row4)
    // createDataFrame 메서드 이용 
    val _df4 = spark.createDataFrame(_data)
    // 암묵적 변환 방식 이용 
    val _df5 = data.toDF

    // 4. 스키마 지정을 통한 데이터프레임 생성 (ex5-21)
    ...
    // 6. 이미지를 이용한 데이터프레임 생성
    val path = s"file:///$sparkHomeDir/data/mllib/images"
    val recursive = true
    val numPartitions = 2
    val dropImageFailures = true
    val sampleRatio = 1.0
    val seed = 0
    val imgdf1 = ImageSchema.readImages(path, spark, recursive, numPartitions, dropImageFailures, sampleRatio, seed)
    val imgdf2 = imgdf1.select($"image.origin", $"image.height", $"image.width", $"image.nChannels", $"image.mode")
    //imgdf1.printSchema
    //imgdf2.show
  }
  ...
  def runAggregateFunctions(spark: SparkSession, df1: DataFrame, df2: DataFrame) {
    ...
    // grouping_id를 이용한 정렬
    df2.cube('store, 'product)
      .agg(sum("amount") as "sum", grouping_id("store", "product") as "gid")
      .filter("gid != '2'")
      .sort(asc_nulls_last("store"), col("gid"))
      .na.fill(Map("store" -> "TOTAL", "product" -> "-"))
  ...
  def runOtherFunctions(spark: SparkSession, personDf: DataFrame) {
    ...
    dd.select('product, 'amount,
      ...
      rank().over(w2).as("rank")).sort("product", "amount").show()
  }
  ...
  // 5.5.2.4.41.4절
  def runRepartitionByRange(spark: SparkSession) {
    import spark.implicits._

    val df = spark.range(0, 500, 7)
    val df1 = df.repartition(5, 'id)
    val df2 = df.repartitionByRange(5, 'id)

    println("Repartition ---------------")
    df1.foreachPartition(it => println(it.toList.mkString("[", ",", "]")))

    println("\n\nRange Partition ---------------")
    df2.foreachPartition(it => println(it.toList.mkString("[", ",", "]")))
  }

  // 5.5.2.4.41.5절
  def runColRegex(spark: SparkSession) {
    import spark.implicits._
    val r1 = ("store2", "note", 20, 2000)
    val r2 = ("store2", "bag", 10, 5000)
    val df = List(r1, r2).toDF("store_nm", "prod_nm", "amount", "price")
    df.select(df.colRegex("`.*nm`")).show()
  }

  // 5.5.2.4.41.6절
  def runUnionByName(spark: SparkSession) {
    import spark.implicits._
    val r1 = ("store2", "note", 20, 2000)
    val r2 = ("store2", "bag", 10, 5000)
    val df1 = List(r1, r2).toDF("store_nm", "prod_nm", "amount", "price")
    val df2 = df1.select("price", "amount", "prod_nm", "store_nm")
    df1.union(df2).show
    df1.unionByName(df2).show
  }

  // 5.5.2.4.41.7절
  def runToJson(spark: SparkSession) {
    import spark.implicits._
    val r1 = ("store2", "note", 20, 2000)
    val r2 = ("store2", "bag", 10, 5000)
    val df = List(r1, r2).toDF("store_nm", "prod_nm", "amount", "price")
    df.select(to_json(struct('store_nm, 'prod_nm, 'amount, 'price)) as ("value")).show(false)
  }

  // 5.5.2.4.41.7절
  def runFromJson(spark: SparkSession) {

    import spark.implicits._

    val v1 = """{"store_nm":"store2", "prod_nm":"note", 
                 "amount":20, "price":2000}"""
    val v2 = """{"store_nm":"store2", "prod_nm":"bag",
    ...
    val options = Map("multiLine" -> "false")
    val df = List(v1, v2).toDF("value")
    df.select(from_json('value, schema, options) as "value").
