...
object KafkaSample {
  ...
  def process(df: DataFrame): DataFrame = {
    // 원하는 연산을 수행( map, filter, groupBy ...)
    df.select(col("value").cast("string").as("value"))
  }
  ...
  // 스트리밍 모드로 카프카 토픽에 저장  
  def writeToKafkaStreamingMode(df: DataFrame): StreamingQuery = {
    df.writeStream // streaming 모드일때는 writeStream, batch 모드일때는 write
      .format("kafka")
      .option("kafka.bootstrap.servers", "localhost:9092")
      .option("topic", "topic3")
      .start()
  ...
  // 배치 모드로 카프카 토픽에 저장 
  def writeToKafkaBatchMode(df: DataFrame) {
